#some effort was put to try and optimize tunable parameters (constansts) for instances on an interval [critical_ratio-ε , critical_ratio+ε].
...
class DMM3SATOptimized:
    def __init__(self, clauses, num_vars, 
                 alpha=7.264952, beta=19.706215, gamma=0.290741, 
                 delta=0.190131, epsilon=0.008693, zeta=0.143817, mu=0.696133,
                 adaptive_dt_factor=1.185345, 
                 avg_scale_factor=2.075337, avg_scale_threshold=0.161275,
                 gamma_decay=0.229501, mu_min=0.634689, mu_growth=0.226800, mu_max=0.946342,
                 seed=GLOBAL_SEED):
        ...
        # Scaling factors that control dynamic adjustments
        self.avg_scale_factor = np.float32(avg_scale_factor)
        self.avg_scale_threshold = np.float32(avg_scale_threshold)
        self.gamma_decay = np.float32(gamma_decay)
        self.mu_min = np.float32(mu_min)
        self.mu_growth = np.float32(mu_growth)
        self.mu_max = np.float32(mu_max)
        ...
        self.adaptive_dt_factor = adaptive_dt_factor  # More aggressive time stepping
    ...
    def _gpu_step(self, dt):
        ...
        avg_scale = 1.0 + max(0, min(2.0, self.avg_scale_factor * (C_mean - self.avg_scale_threshold)))
        
        # Pre-compute all parameters in one batch
        alpha_dynamic = self.alpha0 * math.sqrt(avg_scale)
        beta_dynamic = self.beta0 * avg_scale
        gamma_dynamic = self.gamma0 * (1.0 - self.gamma_decay * step_factor)
        delta_dynamic = self.delta0
        zeta_dynamic = self.zeta0 * (1.0 + step_factor)
        mu_dynamic = min(self.mu_max, self.mu_min + self.mu_growth * step_factor)
